...
export async function GET(request: Request) {
  try {
    ...
    const externalUrl = `${process.env.API_URL}/ocorrencias/curadoria?${params.toString()}`;
    console.log('GET /api/curadoria - URL externa:', externalUrl);
    
    const response = await fetch(externalUrl, { cache: 'no-store' });
    
    if (!response.ok) {
      console.error('GET /api/curadoria - Erro da API externa:', response.status);
      return NextResponse.json({ error: 'Erro ao buscar curadorias' }, { status: response.status });
    }
